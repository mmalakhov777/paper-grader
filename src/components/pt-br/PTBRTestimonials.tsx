...
const PTBRTestimonials: React.FC = () => {
  return (
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-start mb-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-12 lg:grid-cols-3 lg:w-[85%] w-full justify-items-start">
              <FeatureItem
                ...
                title="Feedback esperto por seção"
                description="Tenha insights detalhados para cada parte do seu texto"
              />
              <FeatureItem
                icon={<OneClickFixesIcon className="w-8 h-8" />}
                title="Encontrador de citações"
                description="Valorize seu trabalho com citações direto no texto"
              />
              <FeatureItem
                icon={<CleanDraftIcon className="w-8 h-8" />}
                title="Correções com um clique"
                description="Aplique ajustes na hora, direto no editor, sem complicação"
              />
            </div>
          </div>
...
export default PTBRTestimonials; 